...
class Solution:
    def reconstructQueue(self, people: List[List[int]]) -> List[List[int]]:
        ...
        out = []
        for h, k in people:
            # when inserting, elements in the `out` can only have h being larger or equal to the inserted element
            # thus only need to insert it at the position k, since it bears no effect to element with a larger h
            # and people is sorted by k given the same h, so the inserting point doesn't effect prior element with the same h either
            out.insert(k, [h, k])
        return out
