...
class Solution:
    def minExtraChar(self, s: str, dictionary: List[str]) -> int:
        ...
        for i in range(n-1, -1, -1):
            # initialize to be one more characters, assuming no dictionary word is match
            memo[i] = memo[i+1] + 1
            for word in dictionary:
                w_len = len(word)
                # override the memo[i] if there is a dictionary word match
                if i + w_len <= n and s[i:i+w_len] == word:
                    memo[i] = min(memo[i+w_len], memo[i])
