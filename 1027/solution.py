...
class Solution:
    def longestArithSeqLength(self, nums: List[int]) -> int:
        n = len(nums)
        # the longest sequence by ith, by d difference
        # offset by 500 because the smallest difference is -500, and biggest is 500
        memo = [[1] * 1001 for _ in range(n)]
        ...
        for i in range(n):
            for j in range(i):
                # offset by 500
                d = nums[i] - nums[j] + 500
                memo[i][d] = max(memo[i][d], memo[j][d] + 1)
                max_len = max(max_len, memo[i][d])
