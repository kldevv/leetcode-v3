...
class Solution:
    def widthOfBinaryTree(self, root: Optional[TreeNode]) -> int:
        ...
        while q:
            new_q = []
            # max_width of this level is the first and the last non-None node
            max_width = max(max_width, q[-1][0] - q[0][0] + 1)
            for i, node in q:
                # children of ith in a binary tree is (i*2)th and (i*2+1)th
                if node.left:
                    new_q.append(((i * 2), node.left))
                if node.right:
                    new_q.append(((i * 2 + 1), node.right))
            q = new_q
