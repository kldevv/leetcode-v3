...
class Solution:
    ...
    def maximumSumOfHeights(self, maxHeights: List[int]) -> int:
        ...
        # assume every `i` is the highest
        # fill accum left array sum for `i`
        accum = 0
        # use `-1` to represent an empty array
        incr_by_h = [-1]
        for i in range(n):
            while len(incr_by_h) > 1 and maxHeights[incr_by_h[-1]] > maxHeights[i]:
                j = incr_by_h.pop()
                # `j - incr_by_h[-1]` is the # of elements larger than or equal to `i`
                # and currently filled by `maxHeights[j]`, we need to offset them
                accum -= (j - incr_by_h[-1]) * maxHeights[j]
            # `i - incr_by_h[-1]` is the # of elements larger than or equal to `i` with no values filled
            accum += (i - incr_by_h[-1]) * maxHeights[i]
            accums[i] = accum
            incr_by_h.append(i)

        # fill accum right array sum for `i`
        accum = 0
        # use `n` to represent an empty array
        incr_by_h = [n]
        for i in range(n-1, -1, -1):
            while len(incr_by_h) > 1 and maxHeights[incr_by_h[-1]] > maxHeights[i]:
                j = incr_by_h.pop()
                accum -= (incr_by_h[-1] - j) * maxHeights[j]
            accum += (incr_by_h[-1] - i) * maxHeights[i]
            accums[i] += accum - maxHeights[i]
            incr_by_h.append(i)
