...
class Solution:
    def maxValue(self, n: int, index: int, maxSum: int) -> int:
        leftOffset = index
        rightOffset = (n - 1 - index)

        def isValid(m):
            leftEnd = max(0, m - leftOffset)
            leftSum = (m + leftEnd) * (m - leftEnd + 1) // 2

            rightEnd = max(0, m - rightOffset)
            rightSum = (m + rightEnd) * (m - rightEnd + 1) // 2

            curSum = leftSum + rightSum - m

            return curSum <= maxSum - n

        # lower the degree and make the problem to be array[i] >= 0 instead of >= 1
        i = 0
        j = maxSum - n
        while i < j:
            m = (i + j + 1) // 2
        ...
        # restore the degree by adding one
        return i + 1
