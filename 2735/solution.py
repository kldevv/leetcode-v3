...
class Solution:
    def minCost(self, nums: List[int], x: int) -> int:
        n = len(nums)
        # the best result for performing j times rotation
        cost_for_k_rotate = [x * j for j in range(n)]

        for i in range(n):
            cur_min_cost = nums[i]
            for j in range(n):
                # the optimal cost for i position can be all the possible costs until j rotation
                cur_min_cost = min(cur_min_cost, nums[i - j])
                cost_for_k_rotate[j] += cur_min_cost
        return min(cost_for_k_rotate)
