...
class Solution:
    def maxIncreasingCells(self, mat: List[List[int]]) -> int:
        ...
        for i in range(m):
            for j in range(n):
                val2index[mat[i][j]].append((i, j))

        max_step_cell = [[0] * n for _ in range(m)]
        max_step_group = [0] * (m + n)

        for val in sorted(val2index):
            for i, j in val2index[val]:
                max_step_cell[i][j] = max(
                    max_step_group[i], max_step_group[~j]) + 1
            for i, j in val2index[val]:
                max_step_group[i] = max(max_step_group[i], max_step_cell[i][j])
                max_step_group[~j] = max(
                    max_step_group[~j], max_step_cell[i][j])

        return max(max_step_group)
