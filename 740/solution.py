...
class Solution:
    def deleteAndEarn(self, nums: List[int]) -> int:
        sorted_nums = list(sorted((k, v) for k, v in Counter(nums).items()))
        n = len(sorted_nums)

        memo = [[0] * (n) for _ in range(2)]
        memo[1][0] = sorted_nums[0][0] * sorted_nums[0][1]

        for i in range(1, n):
            memo[0][i] = max(memo[1][i-1], memo[0][i-1])

            value = sorted_nums[i][0] * sorted_nums[i][1]
            if sorted_nums[i][0] - sorted_nums[i-1][0] == 1:
                memo[1][i] = memo[0][i-1] + value
            else:
                memo[1][i] = max(memo[0][i-1], memo[1][i-1]) + value
