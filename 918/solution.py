...
class Solution:
    '''
    There are two cases for what parts of the array output subarray would consist of

    (1) __****__: Pick the middle part
    (2) **___***: Pick the prefix and suffix parts

    Where 
    ***: maximum subarray
    ___: minimum subarray

    Thus, the answer is either the sum of the maximum subarray or the total sum minus the sum of the minimum subarray, 
    with one exception: when all elements are negative, the answer should be the maximum element.
    '''
    ...
    def maxSubarraySumCircular(self, nums: List[int]) -> int:
        ...
        cur_min_sum = 0
        for num in nums:
            # either extend the `cur_max_sum` subarray or reset
            cur_max_sum = max(cur_max_sum + num, num)
            max_sum = max(max_sum, cur_max_sum)

            cur_min_sum = min(cur_min_sum + num, num)
            # either extend the `cur_min_sum` subarray or reset
            min_sum = min(min_sum, cur_min_sum)

        # edge case: all element is negtaive
        if max_sum < 0:
            return max(nums)
